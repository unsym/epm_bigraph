...
class EPMBigraphEnumerator:
    ...
    def _generate_all_EPM_bigraphs(self):
        """
        Generator for all EPM bipartite graphs with (n_q, n_a). The yield is an undirected bipartite graph
        ...
        * n_r = n_q + n_a “R” nodes, each with degree >= self.R_min
        ...
        Q_indices = list(range(self.n_q))
        A_indices = list(range(self.n_q, self.n_q + self.n_a))
        R_indices = list(range(self.n_q + self.n_a, self.n_total))

        # Precompute neighbor options on the right, the edges are treated as unweighted
        two_subsets = list(combinations(R_indices, 2))
        ancilla_subsets = [
            comb
            for d in range(2, self.n_r + 1)
        ...
        for Q_choice in combinations_with_replacement(two_subsets, self.n_q):
            for A_choice in combinations_with_replacement(ancilla_subsets, self.n_a):
                ...
                g = ig.Graph(n=self.n_total, directed=False)
                ...
                for i, qnbrs in enumerate(Q_choice):
                    for r in qnbrs:
                        edges.append((i, r))
                # ancilla edges
                for j, anbrs in enumerate(A_choice, start=self.n_q):
                    for r in anbrs:
                        edges.append((j, r))
                g.add_edges(edges)
    ...
    def enumerate_structural(self, n_q: int, n_a: int, attributes: list[str]=[]) -> dict[tuple, ig.Graph]:
        ...
        * Optionally filters to retain only those graphs whose corresponding
        ...
        self.Gs = {}
        for g in self._generate_all_EPM_bigraphs():
